...
const beforeEditFormModalClose = async (hide: () => unknown) => {
  if (editFormRef.value.isFormHasUnsavedChanges) {
    const agreed = await confirm({
      maxWidth: "380px",
      message:
        "Sono state apportate modifiche non salvate, procedere comunque?",
      size: "small",
    });
    if (agreed) {
      hide();
  ...
  <VaCard>
    <VaCardContent>
      <div class="flex flex-col md:flex-row gap-2 mb-2 justify-between">
        ...
        <VaButton icon="add" @click="router.push({ name: 'schedule' })"
          >Schedula</VaButton
        >
      </div>
    ...
    >
      <h1 v-if="projectToEdit === null" class="va-h5 mb-4">
        Aggiungi schedula
      </h1>
